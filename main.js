...
function sendToBoard(link, winnerName, group, group2) {
	...
	xhttp2.open("POST", "https://habitica.com/api/v3/groups/" + group2 + "/chat", true);
	xhttp2.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhttp2.setRequestHeader("x-api-user", uuid);
	xhttp2.setRequestHeader("x-api-key", apikey);
	xhttp2.send("message=" + fmodmessage);
}
...
function getForm() {
	...
	fMessageGroup2 = x.elements[11].value;
	fHorace = x.elements[12].value;
				...
				if (this.readyState == 4 && this.status == 200 || 201) {
					...
					if(!hasSentMessage) {
						if(fIncludeMessage == "true") {
							sendToBoard(chalID, String(winnerAlias), fMessageGroup, fMessageGroup2);
						} else {
